#!/usr/bin/env python3
"""
Shared utilities: ConvergenceTracker, save_results, set_seed, progress printing.
...
def save_results(phi_hat, num_samples, convergence_history, *, out_dir, tag, dataset, seed):
    """Save phi .npy and JSON with meta + convergence_history. NO time fields."""
...
def maybe_print_progress(tag, s, conv_metric=None, print_interval=100):
    """Print progress: sample count + convergence metric only. No elapsed time."""
    if (s + 1) % print_interval == 0:
        conv_str = (
            f"conv={conv_metric:.4f}"

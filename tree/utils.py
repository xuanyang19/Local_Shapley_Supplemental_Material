...
def save_results(phi_hat, num_samples, convergence_history, *, out_dir, tag, dataset, seed):
    ...
    out = {
        "meta": {"tag": tag, "dataset": dataset, "seed": seed, "num_samples": num_samples, "model": "DecisionTree"},
...
def maybe_print_progress(tag, s, conv_metric=None, print_interval=50):
    if (s + 1) % print_interval == 0:
        conv_str = f"conv={conv_metric:.4f}" if conv_metric is not None and not np.isnan(conv_metric) else "conv=N/A"
